...
describe("profile self-service settings layout", () => {
  ...
  it("keeps theme flavor options as real activatable buttons", async () => {
    ...
    expect(source).toContain("pack.logoAlt");
    expect(source).toContain("<Card");
    expect(source).toContain("grid-cols-[repeat(auto-fit,minmax(min(100%,18rem),22rem))]");
    expect(source).toContain("<CardTitle");
    expect(source).toContain("{pack.label}</CardTitle>");
    ...
    expect(source).toContain('pack.themes.length === 1 && "col-span-2"');
