...
Every cross-model offer, external CLI choice, exact invocation confirmation, skip decision, and user override must use VS Code's native `vscode_askQuestions` tool. Do not write the question only in markdown/plain chat and wait for a reply.

- Use options for `Gemini CLI`, `Codex CLI`, `Manual external review`, and `Skip`.
- Use a second `vscode_askQuestions` call to confirm the exact command/invocation before running any external CLI.
- Never request API keys, tokens, passphrases, or other secrets through `vscode_askQuestions`; ask the user to configure or type secrets directly in the terminal.
...
**Step 1: Ask with `vscode_askQuestions`**

After the single-model review in Step 3 above, but before RECONCILE, pause and ask with `vscode_askQuestions`:
...
5. Mind shell escaping. If the artifact contains quotes, `$(...)`, or backticks, prefer stdin (`echo … | gemini`) or a heredoc over inline `-p "…"`. When in doubt, use `vscode_askQuestions` to confirm the invocation before running it.
