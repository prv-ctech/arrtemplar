...
### 3.A Stack
* **Framework:** React 19 + Vite (SPA, client-rendered). No SSR / RSC in this stack.
  * **INTERACTIVITY ISOLATION:** Any component using Motion, scroll listeners, or pointer physics MUST be an isolated, memoized leaf component so its effects and motion values stay scoped. Keep heavy motion out of layout/shell components.
* **Routing / data:** TanStack Router + TanStack React Query. Use route loaders / query hooks for data; keep components presentational.
* **Styling:** **Tailwind v4** via `@tailwindcss/vite` (this repo's setup). Do NOT add the `tailwindcss` PostCSS plugin. Tailwind v3 only if an existing project demands it.
* **Animation:** **Motion** (the library formerly known as Framer Motion). Import from `motion/react` (`import { motion } from "motion/react"`). The `framer-motion` package still works as a legacy alias - prefer `motion/react` in new code.
* **Fonts:** Self-host with `@font-face` + `font-display: swap` (this stack has no `next/font`). Never link Google Fonts via `<link>` in production.
...
* **Source: Simple Icons** (`https://cdn.simpleicons.org/{slug}/ffffff` for any color, or the `simple-icons` package via `bun add simple-icons`). Covers most known brands.
...
* **LCP** < 2.5s. Hero image must be preloaded (`<link rel="preload" as="image" href="...">`) and rendered with explicit `width`/`height` to avoid CLS. This stack has no `next/image`; preload manually.
...
3. **Code sketch** - minimal working implementation (presentational component default, motion isolated in a memoized leaf).
...
- [ ] **Motion** isolated in memoized leaf components with scoped `useEffect` cleanup?
