...
- Use options for explicit gates such as `Yes`, `No`, and `Refine`, while allowing freeform refinement.
- Do not proceed until `vscode_askQuestions` returns the user's answer.
...
Ask this through `vscode_askQuestions`, then wait for the user to react before asking the next question.
...
Use `vscode_askQuestions` with `Yes`, `No`, and `Refine` options for this gate.
```
